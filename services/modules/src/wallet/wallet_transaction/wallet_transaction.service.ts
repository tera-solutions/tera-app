...
import { WalletTransactionAPI } from "@tera/api/wallet/wallet_transaction/wallet_transaction.api";
...
export const useWalletTransactionList = (payload: ListPayload) => {
  return useQueryAdapter({
    queryKey: ["wallet_transaction", "list", payload.params],
...
export const useWalletTransactionDetail = (payload: DetailPayload) => {
  return useQueryAdapter({
    queryKey: ["wallet_transaction", "detail", payload.id],
...
export const useWalletTransactionCreate = () => {
  ...
  return useMutationAdapter({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["wallet_transaction", "list"] });
    },
  });
};
