...
import { LessonPlanAPI } from "@tera/api/education/lesson_plan/lesson_plan.api";
...
export const useLessonPlanList = (payload: ListPayload) => {
  return useQueryAdapter({
    queryKey: ["lesson_plan", "list", payload.params],
...
export const useLessonPlanDetail = (payload: DetailPayload) => {
  return useQueryAdapter({
    queryKey: ["lesson_plan", "detail", payload.id],
...
export const useLessonPlanCreate = () => {
  ...
  return useMutationAdapter({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lesson_plan", "list"] });
    },
  });
};
