...
> nào) — field name + enum trong `_utils.ts`/`constants.tsx` nay khớp backend. Chi tiết:
> `docs/postman-structure.md` §5.
>
...
9. ~~**⚠️ Tên field response chưa verify.**~~ ✅ **Đã verify bằng response thật 2026-07-09** — xem
   `docs/postman-structure.md` §5. Vẫn chưa verify: `fin/wallet/transactions` có nhận param
...
- **❓ Chưa xác định được resource backend.** Đã kiểm: `Finance` (7 folder) và `HR` (4 folder)
  **không có** "Package"; gần nhất là `Finance/Promotion` (khuyến mãi, voucher) — khác nghiệp vụ.
  ⚠️ **`Education` có 31 folder con chưa được liệt kê tên** (`docs/postman-structure.md` §3) → chưa
...
Verify bằng cách gọi API thật (2026-07-09), kết quả ghi ở `docs/postman-structure.md` §5:
