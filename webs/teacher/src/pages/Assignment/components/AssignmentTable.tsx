...
import type { Assignment } from "../_interface";
...
const AssignmentTable = ({
  ...
  const columns: TableColumn<Assignment>[] = [
    ...
    },
    {
      key: "avatar",
      title: "Ảnh",
      headerClassName: "px-4 py-3 w-14",
    ...
    },
    {
      key: "name",
      title: "Tên bài tập",
      render: (row) => <span className="font-medium text-slate-700">{row.name}</span>,
    },
    {
      key: "class",
      title: "Lớp",
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            type="button"
            title="Xem chi tiết"
            onClick={() => navigate(`${PATHS.assignmentDetail}/${row.id}`)}
