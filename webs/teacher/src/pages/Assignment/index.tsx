...
import type { Assignment, AssignmentStatus } from "./_interface";
import { ASSIGNMENT_STATUS_META } from "./constants";
import { toAssignments, toAssignmentSummary } from "./_utils";
import AssignmentTable from "./components/AssignmentTable";
import AssignmentFilterSidebar, { AssignmentFilterDraft } from "./components/AssignmentFilterSidebar";
...
const AssignmentPage = () => {
  ...
  const assignments = useMemo(() => toAssignments(listQuery.data?.data?.items), [listQuery.data]);
  ...
  const summary = useMemo(() => toAssignmentSummary(summaryQuery.data?.data), [summaryQuery.data]);
  ...
  // by the Evaluation page's class-roster scan.
  const submittedQueries = useQueries({
    queries: assignments.map((item) => ({
      queryKey: ["assignment", "submitted-count", item.id],
      queryFn: () => SubmissionAPI.getSubmitted({ assignmentId: item.id, params: { per_page: 1 } }),
  ...
  const submittedCounts = useMemo(() => {
    ...
    assignments.forEach((item, i) => {
      map[item.id] = (submittedQueries[i]?.data as any)?.data?.pagination?.total ?? 0;
    });
    return map;
  }, [assignments, submittedQueries]);
  const totalSubmitted = Object.values(submittedCounts).reduce((sum, n) => sum + n, 0);
  const totalExpected = assignments.reduce((sum, item) => sum + item.student_count, 0);
  ...
  const handleFilterChange = (patch: Partial<AssignmentFilterDraft>) =>
    setFilters({
  ...
  const openCreateForm = () => navigate(`${PATHS.assignment}/new`);
  const openEditForm = (item: Assignment) => navigate(`${PATHS.assignment}/${item.id}/edit`);
