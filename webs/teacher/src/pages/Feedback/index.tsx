...
import type { StudentFeedbackRow } from "./_interface";
import { toFeedbackSummary, toStudentFeedbackRows } from "./_utils";
import StudentFeedbackTable from "./components/StudentFeedbackTable";
import StudentDetailPanel from "./components/StudentDetailPanel";
import FeedbackForm from "./components/FeedbackForm";

const Feedback = () => {
  const [selectedStudent, setSelectedStudent] = useState<StudentFeedbackRow | null>(null);
  const [formOpen, setFormOpen] = useState(false);
  const [search, setSearch] = useState("");

  const summaryQuery = EvaluationService.useEvaluationStudentSummary();
  const summary = useMemo(
    () => toFeedbackSummary(summaryQuery.data?.data),
    [summaryQuery.data],
  );
  ...
  const rows = useMemo(
    () => toStudentFeedbackRows(studentsQuery.data?.data?.items, evaluationItems, studentClassMap),
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_320px]">
        ...
        <div className="flex flex-col gap-4">
          <StudentDetailPanel
            ...
            onAddFeedback={() => setFormOpen(true)}
          />
        </div>
      </div>
