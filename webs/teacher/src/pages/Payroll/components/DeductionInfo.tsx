...
const DeductionInfo = ({ items, total }: DeductionInfoProps) => (
  <Card className="xmd:p-5" animated={false}>
    <div className="mb-3 flex items-center justify-between gap-2">
      <p className="text-base font-semibold text-slate-800">2. Khấu trừ</p>
      <span className="text-base font-bold text-rose-500">{formatNumber(total)}đ</span>
    </div>
    ...
    <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
      <span className="text-sm font-semibold text-rose-500">Tổng khấu trừ</span>
      <span className="text-base font-bold text-rose-500">{formatNumber(total)}đ</span>
    </div>
  </Card>
);
