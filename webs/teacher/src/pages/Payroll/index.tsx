...
const CURRENT_YEAR =
  parseDmy(PAYROLL_PERIODS[0].toDate)?.getFullYear() ??
  new Date().getFullYear();
const DEFAULT_RANGE: DateRangeValue = {
  from: new Date(CURRENT_YEAR, 0, 1),
  to: new Date(CURRENT_YEAR, 11, 31),
};
...
const Payroll = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return PAYROLL_PERIODS.filter((p) => {
      ...
      if (
        kw &&
        !`${p.period} ${p.incomeType} ${p.fromDate} ${p.toDate}`
          .toLowerCase()
          .includes(kw)
      )
        return false;
  ...
  const handleView = (row: PayrollPeriod) =>
    navigate(`${PATHS.payroll}/${row.id}`);
  const demo = () =>
