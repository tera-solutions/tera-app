...
const EnrollmentUpdatePage = () => {
  ...
  useEffect(() => {
    if (!isMobile) {
      navigate(ENROLLMENT_PAGE_URL.list.path, {
        replace: true,
  ...
  const { data, isPending } = EnrollmentService.useEnrollmentDetail({ id });
        ...
        <div className="bg-white rounded-[5px] w-full p-4">
          <Spin spinning={isPending}>
            <EnrollmentForm
              ...
              ref={actionRef}
              type="update"
              onSuccess={() => navigate(ENROLLMENT_PAGE_URL.list.path)}
            />
          </Spin>
        </div>
...
export default EnrollmentUpdatePage;
