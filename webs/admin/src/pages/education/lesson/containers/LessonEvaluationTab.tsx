...
/** Khối text nhãn–nội dung (điểm mạnh/yếu/đề xuất). */
...
/**
 * Tab Đánh giá trong chi tiết bài học — CHỈ dùng endpoint list, lọc `lesson_id`.
 * Dùng trực tiếp các *_label do API trả; chỉ map MÀU cho classification + status.
 */
const LessonEvaluationTab = ({ lessonId }: { lessonId?: number }) => {
  const { t } = useTranslation();

  const { data, isLoading } = EvaluationService.useEvaluationList({
    params: { lesson_id: lessonId, per_page: 100 },
  });
...
export default LessonEvaluationTab;
