...
import ProductComboView from "./containers/ProductComboView";
import {
  ProductType,
  ProductTypeColor,
} from "@tera/components/shared/Stock/constants/product";
import { convertCombo } from "../../_utils";

const cardClasses = "shadow-none p-0";
const ComboProduct = ({ dataDetail }) => {
  const general = [
    {
      title: "Tên sản phẩm",
  ...
  return (
    <div className="grid gap-5">
      ...
      <div>
        {dataDetail?.type === "combo" && (
          <CardDetailProduct
            title={`Sản phẩm combo ${
              !!dataDetail?.combo?.length && `(${dataDetail?.combo?.length})`
            }`}
          >
            <ProductComboView value={convertCombo(dataDetail?.combo)} />
          </CardDetailProduct>
        )}
      </div>
    </div>
