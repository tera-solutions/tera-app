...
import { ACTIVITY_APPOINTMENT_URL } from "@tera/components/shared/Activity/containers/Appointment/url";
...
function Item(props: IProps) {
  ...
  const items = () => {
    const arrData = [];
    hasPage(QUICK_ACTION_PERMISSION_KEY.DRAWER_APPOINTMENT_VIEW_DETAIL) &&
      arrData.push({
        key: 1,
        label: "Xem",
        onClick: () => {
          navigate(`${ACTIVITY_APPOINTMENT_URL.detail.path}/${value?.id}`);
        },
      });
    return arrData;
  };
  return (
    ...
    >
      ...
      <Description title="Địa điểm" value={value.address} />
      <Description
