...
function Call({ object_type, object_id, isScroll }: CallProps) {
  const {
  ...
  const {
    ...
    queryKey: ["get-list-call", object_type, object_id, keyword],
  ...
  return (
    <Spin spinning={isLoading}>
      ...
      >
        ...
        {hasPage(QUICK_ACTION_PERMISSION_KEY.DRAWER_CALL_SEARCH) && (
          <SearchSort
            handleSearchSort={(value) => setKeyword(value?.keyword)}
            isShowSort
            keyword={keyword}
            placeholder="Tìm kiếm theo tiêu đề nhiệm vụ"
          />
        )}
      </div>
