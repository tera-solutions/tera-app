...
const SelectContact = forwardRef<HTMLSelectElement, SelectDataTypeProps>(
  ({ isCheckAll = false, optionCustom = [], paramsApi, ...props }, ref) => {
    const [search, setSearch] = useState<string>("");
    const searchDebounce = useDebounce(search, 300);

    const { data } = useQuery({
      queryKey: ["get-list-data-contact", searchDebounce, paramsApi],

      queryFn: () => {
        return CrmApi.getContactList({
