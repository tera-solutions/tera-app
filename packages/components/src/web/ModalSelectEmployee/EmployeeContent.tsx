...
const EmployeeContent = (props: IProps, ref) => {
  ...
  const selectEmployeeIds = value?.map((item) => !!item && item.id) ?? [];
    ...
    queryFn: () => {
      return EmployeeApi.getList({
        params: {
          ...
          onlyUser: 1,
          ...(excludeIds && { except_id: excludeIds.join(",") }),
          ...(mode === "delete" && {
            except_id: [...selectEmployeeIds, ...(excludeIds ?? [])].join(","),
          }),
        },
      });
    },
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (jobTitle, record) =>
        record?.employee_hrm ? (
          <div className="line-clamp-2">
            {record?.employee_hrm?.job_title?.name}
          </div>
        ) : (
    ...
    },
    {
      ...
      render: (position, record) =>
        record?.employee_hrm ? (
          <div className="line-clamp-2">
            {record?.employee_hrm?.position?.name}
          </div>
        ) : (
