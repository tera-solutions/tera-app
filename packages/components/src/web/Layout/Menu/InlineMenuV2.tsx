...
const InlineMenuV2: React.FC<InlineMenuProps> = observer(
  ({ isExpand, containerClassName, onChangeSize, module = "sales" }) => {
    const {
    ...
    // 1. Xác định dữ liệu Menu dựa trên Module hiện tại
    ...
    const renderHeading = () => {
      const data = currentModuleData.groupMenu.find(
          ...
          <div className="flex flex-col gap-y-5 items-center h-full">
            ...
            >
              {Object.entries(groupListMenu)
                ...
                .map(([key, value]) => {
                  const groupInfo = currentModuleData?.parentGroup?.find(
                    (i: any) => i.key === key,
                  );
