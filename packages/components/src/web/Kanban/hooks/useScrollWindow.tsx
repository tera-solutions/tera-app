...
const startScroll = (type: IScrollType, speed: number) => {
  const typeObject = {
    toLeft: [-1 * speed, 0],
    toRight: [speed, 0],
    toBottom: [0, speed],
  ...
  return setInterval(() => window.scrollBy(value[0], value[1]), 20);
};

interface IProps {
  speed?: number;
}
const useScrollWindow = (value: IProps) => {
  const { speed = 15 } = value;
  ...
  useEffect(() => {
    ...
    if (scrollType !== "none") {
      clearInterval(intervalId);
      intervalId = startScroll(scrollType, speed);
    }
    return () => {
      clearInterval(intervalId);
    };
