...
function ModalSelectActivityAppointment() {
  const {
    modalSelectStore: { appointment, closeModal, updateData },
  } = useStores();
  // const {
  ...
  const { data: listDataTable, isLoading } = useQuery({
    queryKey: [
      "get-list-select-activity-appointment-modal",
      pagination,
      searchKeyword,
    ],
  ...
  const handleOk = () => {
    const convertData = rowSelected?.map((item) => ({
      ...
      idAppointment: item?.id || item?.idAppointment,
      title: item?.title,
      status: item?.status,
      status_text: item?.status_text,
    }));
    updateData("appointment", "data", convertData);
  ...
  useEffect(() => {
    if (appointment?.data?.length > 0) {
      const listId = appointment?.data.map((item) => item?.idAppointment);
      setRowId(listId);
      setRowSelected(appointment?.data);
    }
  }, []);
  ...
  return (
    <Modal
      ...
      onOk={() => handleOk()}
      onCancel={() => closeModal("appointment")}
        ...
        <div className="flex justify-between items-center pb-4">
          <div>
            <SelectContactModalHeader
              ...
              placeholderProp="Tìm kiếm theo mã, tên lịch hẹn"
            />
          </div>
        </div>
        <Row>
          ...
          onRow={(record: any) => ({
            onClick: () => {
              ...
              const filterData = rowSelected.filter(
                (item) =>
                  item?.id !== record?.id && item?.idAppointment !== record?.id,
              );
              const filterId = filterData.map(
                (item) => item?.id || item?.idAppointment,
              );
...
export default observer(ModalSelectActivityAppointment);
